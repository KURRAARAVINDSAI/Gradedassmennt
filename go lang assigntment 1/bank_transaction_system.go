...
func (bs *BankSystem) Deposit(id int, amount float64) error {
	...
	transaction := fmt.Sprintf("%s: +$%.2f (Balance: $%.2f) - %s",
...
func (bs *BankSystem) Withdraw(id int, amount float64) error {
	...
	if account.Balance < amount {
		return fmt.Errorf("insufficient balance. Current balance: $%.2f", account.Balance)
	}
...
func (bs *BankSystem) RunMenu() {
	...
	account, err := bs.CreateAccount(1, "John Doe")
	if err != nil {
